$(function () {
    layui.use(['layer','laypage','element','laydate','form'], function(){
        ...
        $(document).on("click",".updatestate",function () {
            var topicId = $(this).parent().parent().attr("id");
            topicIdstatus = topicId;
            var status = $(this).parent().parent().children("td:nth-child(7)").text();
        ...
        $(document).on("click",".status-update-btn",function () {
            var topicId = topicIdstatus;
            var status = $("#status-select").val();
            if(status==''){
                layer.msg("请选择一个状态", {icon: 5,anim: 6});
                return false;
            }
            var data = 'topicId='+topicId+"&status="+status
            $.ajax({
                url:'/admin/forum/updateTopicStatus',
                data:data,
                type:'post',
                success:function (resp) {
                    if (resp.code==0) {
                        ...
                        tr.children("td:nth-child(7)").text(text);
                    } else {
                        layer.msg(resp.msg, {icon: 5,anim: 6});
                    }
                    return false;
